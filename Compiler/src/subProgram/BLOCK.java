...
public class BLOCK {
    ...
    private PrintWriter syntaxTreeFile;
    ...
    public boolean program(String treeFile) throws IOException {
        ...
        Code.addCode("opr", 0, Code.getCodeIndex("returnOpr"));
        cx++;
    ...
    private boolean subProgram() {
        ...
        Code tempCode = new Code("jmp", 0, cx);
        Code.setCode(tempCodeAddr, tempCode);
        Code.addCode("int", 0, varCount-tempVarCount+3);
        cx++;
    ...
    private boolean sentence() {
        ...
        if (kind().equals(";_bd")) {            // 空语句
            ...
            depth -= 2;
            pos++;
            int labelAddr = indexOfSynTable(tempName);
            if (labelAddr == -1)
                throw new ErrorException("error in code at line "+line());
            int diffLevel = Math.abs(synTable.synTables.get(labelAddr).getLevel()-proLevel);
            if (diffLevel == 3)
                diffLevel--;
            Code.addCode("sto", diffLevel, synTable.synTables.get(labelAddr).getAddr());
            cx++;
        }
            ...
            cx++;
            sentence();
            Code.addCode("jmp", 0, tempAddr);
            cx++;
            Code.setCode(tempAddr1, new Code("jpc", 0, cx));
            depth--;
        }
            ...
            syntaxTreeFile.println(deep()+name());
            pos++;
            int labelAddr = indexOfSynTable(name());
            if (labelAddr == -1)
                throw new ErrorException("error in call at line "+line());
            if (!isIdentifier())
                throw new ErrorException("error: '"+name()+"' is not a identifier at line "+line());
            if (!kind().equals(";_bd"))
                throw new ErrorException("error: need a ';' after call at line "+line());
            pos++;
            int entryCode = Code.getOffset(synTable.synTables.get(labelAddr).getAddr());
            int ttt = Math.abs(synTable.synTables.get(labelAddr).getLevel()-proLevel);
            if (ttt == 3)
                ttt--;
            Code.addCode("cal", ttt, entryCode);
            cx++;
            depth -= 2;
        }
            ...
            depth++;
            syntaxTreeFile.println(deep()+name());
            Code.addCode("opr", 0, Code.getCodeIndex("readOpr"));
            cx++;
            pos++;
            if (!kind().equals("(_bd"))
                throw new ErrorException("error: need a '(' after call at line "+line());
            ...
            int labelAddr = indexOfSynTable(name());
            if (labelAddr == -1)
                throw new ErrorException("error in read at line "+line());
            int ttt = Math.abs(synTable.synTables.get(labelAddr).getLevel()-proLevel);
            if (ttt == 3)
                ttt--;
            Code.addCode("sto", ttt,
                    synTable.synTables.get(labelAddr).getAddr());
            cx++;
            if (!isIdentifier())
                throw new ErrorException("error: '"+name()+"' is not a identifier at line "+line());
            while (kind().equals(",_bd")) {
                ...
                Code.addCode("opr", 0, Code.getCodeIndex("readOpr"));
                cx++;
                labelAddr = indexOfSynTable(name());
                if (labelAddr == -1)
                    throw new ErrorException("error in read at line "+line());
                int ttt1 = Math.abs(synTable.synTables.get(labelAddr).getLevel()-proLevel);
                if (ttt1 == 3)
                    ttt1--;
                Code.addCode("sto", ttt1,
                        synTable.synTables.get(labelAddr).getAddr());
                cx++;
                if (!isIdentifier())
                    throw new ErrorException("error: '"+name()+"' is not a identifier at line "+line());
            ...
            pos++;
            expression();
            Code.addCode("opr", 0, Code.getCodeIndex("writeOpr"));
            cx++;
            Code.addCode("opr", 0, Code.getCodeIndex("chaLineOpr"));
            cx++;
            while (kind().equals(",_bd")) {
                syntaxTreeFile.println(deep()+name());
                pos++;
    ...
    private int expression() {
        ...
        if (kind().equals("-_op")) {
            ...
            pos++;
            exp = -term();
            Code.addCode("opr", 0, Code.getCodeIndex("subOpr"));
            cx++;
        }
            ...
            pos++;
            exp = term();
            Code.addCode("opr", 0, Code.getCodeIndex("addOpr"));
            cx++;
        }
        else exp = term();
    ...
    private int term() {
        ...
        while (kind().equals("*_op") || kind().equals("/_op")) {
            if (kind().equals("*_op")) {
                ...
                pos++;
                ter *= factor();
                Code.addCode("opr", 0, Code.getCodeIndex("mulOpr"));
                cx++;
            }
            else {
                syntaxTreeFile.println(deep()+name());
                pos++;
                ter /= factor();
                Code.addCode("opr", 0, Code.getCodeIndex("divOpr"));
                cx++;
            }
        }
        depth--;
    ...
    private int factor() {
        ...
        if (Variable.isVariable(name())) {// 因子是标识符
            int labelAddr = indexOfSynTable(name());
            if (labelAddr == -1)
                throw new ErrorException("error in factor at line "+line());
            if (synTable.synTables.get(labelAddr).getKind().equals("const"))
                Code.addCode("lit", 0, synTable.synTables.get(labelAddr).getValue());
            else if (synTable.synTables.get(labelAddr).getKind().equals("var")) {
                int ttt = Math.abs(synTable.synTables.get(labelAddr).getLevel()-proLevel);
                if (ttt == 3)
                    ttt--;
                Code.addCode("lod", ttt,
                        synTable.synTables.get(labelAddr).getAddr());
            }
            cx++;
    ...
    private boolean condition() {
        ...
        if (kind().equals("odd_Re")) {
            ...
            pos++;
            int con = expression();
            Code.addCode("opr", 0, Code.getCodeIndex("oddOpr"));
            cx++;
            depth--;
            return con == 0;
        }
            ...
            depth--;
            if (op.equals("=_op")) {
                Code.addCode("opr", 0, Code.getCodeIndex("equOpr"));
                cx++;
                return con1 == con2;
            }
            else if (op.equals("#_op")) {
                Code.addCode("opr", 0, Code.getCodeIndex("neqOpr"));
                cx++;
                return con1 != con2;
            }
            else if (op.equals("<_op")) {
                Code.addCode("opr", 0, Code.getCodeIndex("lessOpr"));
                cx++;
                return con1 < con2;
            }
            else if (op.equals("<=_op")) {
                Code.addCode("opr", 0, Code.getCodeIndex("elessOpr"));
                cx++;
                return con1 <= con2;
            }
            else if (op.equals(">_op")) {
                Code.addCode("opr", 0, Code.getCodeIndex("greOpr"));
                cx++;
                return con1 > con2;
            }
            else if (op.equals(">= op")) {
                Code.addCode("opr", 0, Code.getCodeIndex("egreOpr"));
                cx++;
                return con1 >= con2;
            }
        }
    ...
    private int indexOfSynTable(String s) {
        for (int i = synTable.synTables.size()-1; i >= 0; i--) {
            if (s.equals(synTable.synTables.get(i).getName()))
                return i;
        }
        return -1;
    }
